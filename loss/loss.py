...
class LossProvider(nn.Module):
    def __init__(self, loss_weights: list, device):
        ...
        loss_percep.load_state_dict(torch.load(
            './loss/rgb_watson_vgg_trial0.pth', map_location='cpu', weights_only=False))
        loss_percep = loss_percep.to(device)
        self.loss_per = lambda pred_img, gt_img: loss_percep(
            (1+pred_img)/2.0, (1+gt_img)/2.0) / pred_img.shape[0]

    def __call__(self, pred_img_tensor, gt_img_tensor, init_latents, wm_pipe):
        init_latents_fft = torch.fft.fftshift(
            torch.fft.fft2(init_latents), dim=(-1, -2))
        ...
        lossI = self.loss_img(
            pred_img_tensor, gt_img_tensor)*self.loss_weights[0]
        lossP = self.loss_per(
            pred_img_tensor, gt_img_tensor)*self.loss_weights[1]
