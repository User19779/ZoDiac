...
def center_crop(img_to_crop: Image.Image, output_size):
    # 获取图像的原始尺寸
    ...
    new_img.paste(img_to_crop, paste_position)
    return new_img


def mask_img_tensor(
        img_tensor, mask: str, mask_value: int = 255,
        make_square: bool = True):
    img_mask = Image.open(mask).convert("1")

    img_mask = img_mask
    crop_size = img_tensor.shape[-1:-3:-1]
    img_mask = center_crop(img_mask, crop_size)
...
def get_img_tensor(
    img_path, device, mask: Optional[str] = None,
    mask_value: int = 255,
    make_square: bool = True
):
    img = Image.open(img_path).convert("RGB")
    img_square = _make_square(img,) if make_square else img
    img_tensor = pil_to_tensor(img_square)

    if bool(mask):
        img_tensor = mask_img_tensor(
            img_tensor, mask, mask_value, make_square=make_square)
